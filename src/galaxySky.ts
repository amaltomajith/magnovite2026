...
export function initGalaxySky() {
  ...
  interface Curtain {
    ...
    driftSpd: number; driftPh: number; driftW: number;
  }
...
// ============================================================
// NEBULA CANVAS — Canvas Engine v2 (Optimized)
// Layered cloud/nebula nodes with drifting stars.
// Default target updated to '.about-hero-section' to avoid conflicts.
// ============================================================

export function initNebulaCanvas(targetSelector = '.about-hero-section') {
  const heroSection = document.querySelector(targetSelector) as HTMLElement;
  if (!heroSection) return;
  if (heroSection.querySelector('.nebula-canvas')) return;
